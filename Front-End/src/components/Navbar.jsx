...
function Navbar() {
  ...
  const { setShowSearch, getCartCount, navigate,isAuthenticated } = useContext(StoreContext)
  const Logout=()=>{
    ...
    <nav className='flex item-center justify-between py-5 font-medium'>
      ...
      <div className='flex items-center gap-5 cursor-pointer'>
        ...
        <div className="group relative">
          ...
          <img onClick={()=>loginCheck()} src={assets.profile_icon} className='w-5 cursor-pointer' alt="" />
          
          <div className='group-hover:block hidden absolute right-0 drop-down-menu pt-4'>
            <div className='flex flex-col gap-2 w-36 py-3 px-5 bg-slate-100 text-gray-500 rounded'>
              <p onClick={()=>navigate('/myprofile')} className='cursto-pointer hover:text-black'>My Profile</p>
              <p onClick={() => navigate('/orders')} className='cursor-pointer hover:text-black'>
                My Orders
              </p>
              <p onClick={()=>Logout()} className='cursto-pointer hover:text-black'>Logout</p>
            </div>
          </div>
        </div>
